...
<body>
    ...
    <form action="{{ route('updateEmployees.edit', ['id' => $employee->id]) }}" method="post">
        ...
        <select name="role" required>
            <option value="Senior Stylist" {{ $employee->role === 'Senior Stylist' ? 'selected' : '' }}>Senior Stylist</option>
            <option value="Junior Stylist" {{ $employee->role === 'Junior Stylist' ? 'selected' : '' }}>Junior Stylist</option>
        </select>
        <br>
